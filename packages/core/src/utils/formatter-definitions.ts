...
{live.deliveryFormatLabel::exists["{live.deliveryFormatLabel}"||""]}{stream.visualTags::exists[" | {stream.visualTags::join(' | ')}"||""]}`,
...
{stream.message::exists["Message: {stream.message}"||""]}`,
  ...
  gdrive: {
    name: `{stream.proxied::istrue["🕵️ "||""]}{live.channelName::exists["{live.channelName}"||"{addon.name}"]} {stream.resolution::exists["{stream.resolution}"||""]}{live.deliveryFormatLabel::exists[" · {live.deliveryFormatLabel}"||""]}{stream.regexMatched::exists[" ({stream.regexMatched})"||""]}`,
...
{stream.message::exists["ℹ️ {stream.message}"||""]}`,
  ...
  minimalisticgdrive: {
    name: `{stream.resolution::exists["{stream.resolution::replace('2160p','✨ 4K')::replace('1440p','📀 2K')::replace('1080p','🧿1080p')::replace('720p','💿720p')}"||"LIVE"]}{live.deliveryFormatLabel::exists["  {live.deliveryFormatLabel}"||""]}
...
`,
  ...
  prism: {
    ...
    description: `{live.channelName::exists["📺 {live.channelName} "||""]}{live.deliveryFormatLabel::exists["🎚️ {live.deliveryFormatLabel} "||""]}{stream.regexMatched::exists["🎚️ {stream.regexMatched} "||""]}
...
`,
  ...
  tamtaro: {
    ...
    description: `{live.providerName::exists["✎  {live.providerName::truncate(15)}"||""]}{live.deliveryFormatLabel::exists["  {live.deliveryFormatLabel::smallcaps}"||""]}
