...
description: A complete reference for every option on the AIOLiveTV configuration page.
---

<Callout type="info">
  For the Live TV first run, start with the [Setup Guide](/configuration/setup),
  then [XMLTV, M3U and Xtream](/guides/sources) and [Channels](/guides/channels).
  This page is the leftover option-by-option reference for filters, sorting and
  formatter fields that still apply to custom live addons.
</Callout>

### Description

This addon lets you combine live TV sources into one Stremio addon, with consistent formatting and filtering of the results.
...
need to click the big white `Configure` button on AIOLiveTV's configuration page.
...
required and is a feature meant for advanced users. You may join the [GitHub Issues](https://github.com/mrcanelas/aiolivetv/issues) to view the community's configurations.
...
are all provided for convenience's sake to ensure you can easily add addons without having to leave AIOLiveTV.
...
- `Name` - This lets you change the name of the addon as it appears in AIOLiveTV. This name is only used for stream results, it does not affect any catalog names.
- `Timeout` - This lets you change the time that AIOLiveTV waits for the addon to respond before it aborts the request to it. Used for all requests to the addon, including catalogues, streams, subtitles etc.
- `Resources` - This option allows you to override the resources that AIOLiveTV would use from the addon. This is useful if you want to disable catalogues from the addon, but keep its streams. So you would only select `stream` here. In most cases, its recommended to leave this blank, or at the defaults - as it is automatically determined by AIOLiveTV later.
- `URL` - In most cases, you will be able to override the URL of the addon here. Usually this is generated automatically by AIOLiveTV - A benefit of this is you never having to worry about other addon updates, as AIOLiveTV will always use the latest version of the addon. However, if you want to use a custom URL, you can do so here. This is useful for using a specific configuration of the addon. Note: you will need to configure any debrid services separately, debrid services configured in AIOLiveTV do not apply to overriden URLs.
...
The [Wiki](/guides/groups) has more information on how to use this feature.

<h3>Filters</h3>

One of AIOLiveTV's most recongisable feature is its advanced filtering system. This menu may look a bit overwhelming, but it is neatly organised into sections
...
<Callout type="info">
  ...
  required via `AIOLIVETV_AUTH` / `AIOLIVETV_AUTH_REQUIRED`) so no one
...
<h4>Result Limits</h4>
This section allows you to limit the number of results returned by AIOLiveTV.
...
What this means is that AIOLiveTV will cut off streams after a specific group limit is reached. This means that it would keep streams
...
You can either choose from a list of pre-defined formats, or create your own custom format using AIOLiveTV's [Formatter Syntax](/reference/custom-formatter)

<Callout type="info">
  Join AIOLiveTV's [GitHub Issues](https://github.com/mrcanelas/aiolivetv/issues) to see the formats
...
You can use this section to optionally configure a stream proxy for AIOLiveTV.
...
Here, you will be able to create a new user on your AIOLiveTV instance, or update the existing user with the current configuration.
...
Note that while this option filters out all sensitive information entered in AIOLiveTV, it will not filter out any URLs from custom addons or URLs that you have overriden.
...
You may also manually go to the configuration page by navigating to `https://your-instance-url/stremio/configure` where `your-instance-url` is the URL of your AIOLiveTV instance,
...
If you need help in configuring this addon, feel free to ask in the [GitHub Issues](https://github.com/mrcanelas/aiolivetv/issues).
