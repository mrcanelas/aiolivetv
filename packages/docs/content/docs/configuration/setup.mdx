...
<Callout type="warn">
  AIOLiveTV is in **public beta**. The with/without EPG flow, Native EPG
  declaration and the Channels page work. Expect breaking changes before 1.0.
</Callout>
...
| Brazilian EPG | Vivo TV or Claro TV+, plus an M3U/Xtream for streams |
...
XMLTV and Xtream can shift programme times with **EPG Time Shift**. Local playlists on a LAN need `HTTP_ALLOW_PRIVATE_URLS=true` on a personal instance — leave it `false` on anything shared.
