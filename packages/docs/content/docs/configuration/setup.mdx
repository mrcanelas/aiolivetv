---
title: Setup Guide
description: Get AIOLiveTV up and running from scratch using Tamtaro's complete setup.
---

This guide walks you through a complete AIOLiveTV setup — from installing your media client to having a fully configured addon ready to use. It uses a curated community template that handles the bulk of the configuration so you only need to fill in your credentials.
...
which bring the Trash Guides release group rankings into AIOLiveTV.
...
AIOLiveTV is primarily designed and maintained for Stremio, but works with other Stremio addon compatible clients as well
...
<Tabs items={['Stremio', 'Other Clients']}>
  ...
  <Tab value="Other Clients">
    The following clients also work with AIOLiveTV — most via Stremio addon
    ...
    <Callout type="info">
      **Chillio users:** when installing AIOLiveTV, select **Chill Link
      Protocol** (not Stremio) in the install modal.
    </Callout>
    <Callout type="info">
      **Seanime users:** AIOLiveTV provides dedicated extensions for Seanime.
        ...
        **Setup:**
        ...
        <Steps>
          ...
          <Step>
            ...
            it during AIOLiveTV configuration.
          </Step>
        </Steps>
      </Tab>
...
- In **Stremio**, click **Configure** next to the AIOLiveTV addon.
...
Run into something not covered here? Open a [GitHub Issue](https://github.com/mrcanelas/aiolivetv/issues).
