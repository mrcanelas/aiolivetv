...
| `AIOSTREAMS_USER_AGENT` | Aiostreams User Agent | string | `AIOLiveTV/{version}` | User-Agent identifying this addon to upstream services. Supports `{version}` and `{random}` placeholders. _(restart required)_ |
