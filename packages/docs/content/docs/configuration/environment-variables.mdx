...
| `HTTP_ALLOW_PRIVATE_URLS`   | `false`                     | Allow outbound HTTP to localhost and private/reserved IP ranges (RFC1918, link-local, cloud metadata). Leave `false` on public or multi-user instances. Set `true` only for a personal LAN that hosts XMLTV, M3U or Prowlarr locally.                                                                        |
