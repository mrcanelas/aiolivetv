...
Search for streams across all enabled and configured sources in your AIOLiveTV instance. Results are processed through the same filtering and sorting pipeline as normal Stremio stream requests.
...
- [UserData schema](https://github.com/mrcanelas/aiolivetv/blob/main/packages/core/src/db/schemas.ts) — the JSON object encoded in `x-aiostreams-user-data`
- [SearchResult schema](https://github.com/mrcanelas/aiolivetv/blob/main/packages/core/src/transformers/api.ts) — the shape of each result in `data.results`
