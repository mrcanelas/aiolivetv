...
Resolves an anime identifier from one database to its mappings across other supported databases. Uses the internal AIOLiveTV anime database (aggregated from AniDB, AniList, TVDB, TMDB, MAL, Kitsu, and more).
