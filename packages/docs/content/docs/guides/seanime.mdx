...
description: A guide on using AIOLiveTV in Seanime.
---
...
AIOLiveTV comes with three extensions that integrate directly with Seanime.
...
preserves all your AIOLiveTV sorting and formatting exactly as configured. You can open the panel from a variety of
triggers; by default it adds an **AIOLiveTV** tab alongside the standard **Torrent Streaming** / **Online Streaming** /
...
### AIOLiveTV Torrent Provider

Adds AIOLiveTV as a torrent provider inside Seanime's **Torrent Streaming** / **Debrid Streaming** tab. Only the
torrents surfaced by AIOLiveTV are used - any debrid configured in AIOLiveTV itself is ignored. Seanime's own debrid
...
<Callout type="info">
  ...
  Provider may be deprecated in a future release. The **AIOLiveTV Plugin** is preferred.
</Callout>
...
movie/series ones. It does not provide streams on its own; pair it with the **AIOLiveTV Plugin**, which can serve
...
<Steps>
  <Step>
    Configure AIOLiveTV as normal (e.g. using [the setup guide](/configuration/setup/)).
  </Step>
  <Step>
...
<Tabs items={['Plugin', 'Torrent Provider', 'Stremio Custom Source']}>
  ...
  <Tab value="Torrent Provider">
    <Steps>
      <Step>
        Copy the **Manifest URL** for the `AIOLiveTV Torrent Provider` shown on the Seanime pop-up.
      </Step>
      <Step>
  ...
  <Tab value="Stremio Custom Source">
    ...
    <Steps>
      ...
      <Step>
        Enter a Stremio addon manifest URL to import catalogues from. If your catalogues are already in AIOLiveTV, click
