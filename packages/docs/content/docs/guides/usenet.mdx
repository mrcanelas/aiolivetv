...
**AIOLiveTV Proxy (Newznab):** Configure the Newznab addon to proxy NZB grabs through AIOLiveTV by providing your `AIOLIVETV_AUTH` credentials in the addon settings (`user:password`). This makes both the query and grab come from the same AIOLiveTV IP.
