---
title: Templates
description: Complete reference for writing AIOLiveTV configuration templates.
---
...
A template is a shareable JSON file that applies a partial AIOLiveTV configuration to a user's instance. Templates support dynamic expressions, user inputs, and service-conditional logic — making them powerful enough to cover most configuration scenarios without hardcoding anything.

<Callout type="warn">
  AIOLiveTV does not ship torrent or Usenet templates. Examples below that
  mention Torrentio, GDrive or debrid services describe the template *engine*,
  not the marketplace. Prefer templates that set XMLTV, M3U, Xtream or live
  addons.
</Callout>
...
AIOLiveTV parses this file with a strict format:
...
Send a user directly to the template import flow by appending query parameters to any AIOLiveTV URL:

```
https://your-aiolivetv.example.com/stremio/configure?template=https://example.com/my-template.json
```

With a specific template pre-selected from a multi-template file:

```
https://your-aiolivetv.example.com/stremio/configure?template=https://example.com/templates.json&templateId=author.my-template
```
