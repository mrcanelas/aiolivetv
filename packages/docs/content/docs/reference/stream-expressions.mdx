...
The **Stream Expression Language (SEL)** lets you write custom logic to filter, rank, and select streams. In AIOLiveTV it is used mainly in **Groups** to skip backup live addons when the primary sources already returned enough streams.

<Callout type="warn">
  Operators such as `seadex()`, `cached()`, `isAnime` and torrent indexer names
  come from the inherited AIOStreams pipeline. They are documented here because
  the engine still evaluates them; they are not Live TV features.
</Callout>
