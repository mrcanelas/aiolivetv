...
<Callout type="info">
  For live TV, `{addon.name}`, `{stream.type}` (`live` / `http`) and
  `{stream.message}` matter most. Debrid, Usenet and SeaDex fields still exist
  in the engine and in some built-in formats, but they stay empty for live
  streams.
</Callout>
...
Field and modifier names are case-insensitive. Whitespace inside braces is
ignored. Quoted literals can be used anywhere a field is expected:
...
Wrap text in `{? ... ?}` so it only renders when every field inside is present.
Surrounding literals are dropped with the group:

```
{addon.name}{? · {stream.resolution} ?}
```

With resolution `1080p` this becomes `Vivo TV · 1080p`. Without resolution it
becomes `Vivo TV`.

### Missing vs false

Conditionals accept a third branch for a missing value, distinct from false:

```
{stream.resolution::exists["{stream.resolution}"||"unknown"||"no resolution"]}
```

`::default('X')` fills a missing value without a full conditional:

```
{stream.resolution::default('SD')}
```
...
| -------------------- | ------ | ---------------------------------------------------------- |
| `{config.addonName}` | string | The name of the AIOLiveTV instance (Branding → Addon name; `ADDON_NAME`) |
...
| `::default('X')`                   | Use `X` when the value is missing                 |
| `::in('a','b')`                    | True when the value is one of the listed options  |
| `::translate('from','to')`         | Replace matching values                           |
| `::date('%B %o, %Y')`              | Format an ISO date (`%Y` `%m` `%d` `%B` `%o` …)   |
| `::languagecode`                   | Convert a language name to an ISO code            |
| `::languageemoji`                  | Convert a language name to a flag emoji           |
| `::subscript` / `::superscript`    | Convert digits/letters to sub/superscript         |
...
| `::time('%H:%M')`         | Pattern duration (`%H` `%M` `%S`; `[...]` drops zeros) |
...
Community-created formats are shared on the [GitHub Issues](https://github.com/mrcanelas/aiolivetv/issues).

You can also view the built-in formatter definitions at:
[`packages/core/src/formatters/predefined.ts`](https://github.com/mrcanelas/aiolivetv/blob/main/packages/core/src/formatters/predefined.ts)
