...
<Accordions>
  ...
  <Accordion title="Stremio has no programme guide">
    Native EPG is declared only when a guide source (XMLTV, Xtream with EPG, Vivo, Claro) is enabled with **Catalog**. M3U alone never sets `behaviorHints.epgProvider`.
  </Accordion>
...
<Accordions>
  <Accordion title="Results appear in standalone addons but not in AIOLiveTV">
  ...
  <Accordion title="AIOLiveTV is returning results for the wrong season, episode, or movie">
  ...
  <Accordion title="Results are very slow to appear">
    AIOLiveTV waits for all configured addons to respond before returning results. If one addon is slow, everything is delayed.
...
<Accordions>
  ...
  <Accordion title="TMDB API calls fail on my VPS (India / Oracle Cloud)">
    ...
    **Fix:** Route TMDB calls through WARP (or another proxy):
    ...
    2. Add `api.themoviedb.org:true` to the proxy list in your AIOLiveTV `.env`.
  ...
  <Accordion title="'Forbidden regex' error in filter configuration">
    **Cause:** AIOLiveTV restricts arbitrary regex patterns in filters by default.
